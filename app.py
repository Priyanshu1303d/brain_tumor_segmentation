...
def load_model():
    if not os.path.exists(MODEL_PATH):
        st.warning(
            f"Model file not found at {MODEL_PATH}. Please ensure the model exists before proceeding."
        )
        return None
                ...
                try:
                    ...
                    for i, p in enumerate(ordered_paths):
                        try:
                            ...
                            st.error(
                                f"Error processing {os.path.basename(p)}: {str(e)}"
                            )
                            st.stop()

                    # Create input tensor
                    image = torch.cat(images, dim=0).unsqueeze(
                        0
                    )  # Shape: (1, 4, H, W, D)
                    ...
                    if ref_img.shape[2] > 0:  # Make sure there's a valid Z dimension
                            ...
                            with view_col:
                                ...
                                if show_overlay:
                                    ...
                                    for class_idx in range(
                                        ...
                                        if (
                                            class_idx in class_visibility
                                            and class_visibility[class_idx]
                                        ):
                                            ...
                                            colored_mask[..., 3] = (
                                                colored_mask[..., 3] * overlay_opacity
                                            )
                        ...
                        with stats_tab:
                            ...
                            if tumor_present:
                                ...
                                class_counts = {
                                    int(c): np.sum(pred == c) for c in classes
                                }
                                ...
                                class_values = [
                                    class_counts[int(c)] for c in classes if c > 0
                                ]

                                fig, ax = plt.subplots(figsize=(10, 6))
                                colors = [
                                    CLASS_COLORS[i][:3] for i in classes if i > 0
                                ]  # RGB part of RGBA
                                ax.pie(
                                    class_values,
                                    labels=class_labels,
    ...
    if all_files:
        ...
        for file in all_files:
            if is_valid_nifti(file):
                ...
                st.warning(
                    f"{file.name} is not a valid NIfTI file and will be ignored."
                )

        if len(file_assignments) == 4 and all(
            mod in file_assignments for mod in MODALITY_NAMES
        ):
...
st.markdown("---")
st.markdown(
    "**Brain Tumor Segmentation Project** • Powered by MONAI & Streamlit • Report any [issues](https://github.com/sanskarmodi8/brain_tumor_segmentation/issues)"
)
